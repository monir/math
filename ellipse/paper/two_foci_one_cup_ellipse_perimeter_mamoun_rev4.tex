...
\textbf{The logatom framework is developed in standard analysis;
EML is a parallel translation, not a prerequisite.}  Everything
above --- the logatom primitives, the Gamma-moment identity, the
Heisenberg ladder, the augmented basis, the Bernstein convergence,
and the Walsh--Bernstein separation theorem of \S~\ref{thm:separation}
--- is developed in classical real analysis: Chebyshev expansions,
Bernstein's geometric-rate theorem, Heisenberg-ladder identities on
Laguerre frames, and the closed-form Gamma moments of
$x^j e^{-\alpha x}$.  Nothing in UNDERWOOD's main results requires a
new grammar to state, prove, or use.

After the framework was in place, the author observed that the
logatoms also inhabit a recently announced \emph{alternative
encoding} of the elementary functions: Odrzywo{\l}ek's
\textbf{EML} (\emph{Effective Mathematical Language}) result
...
built by a finite EML tree from $\{1, \mathrm{eml}\}$, with the
Catalan numbers counting trees by size.  The logatoms
$\xi^{\alpha}(\log 1/\xi)^{j}$ turn out to be the depth-$\le 3$
EML expressions relevant to log-branch approximation, and the
Gamma-moment map
\[
  \Phi(\alpha, j) \;:=\; \frac{j!}{(\alpha+1)^{j+1}}
\]
suggests a syntax-free identification of logatom-equivalent EML
trees.  This is a \emph{parallel description of objects already
defined in standard analysis}, not a substrate that UNDERWOOD's
proofs depend on.  Whether the EML viewpoint yields algorithmically
or theoretically novel insights for log-branch work is treated in
this paper as an \emph{open research direction}, with any
exploratory observations gathered in a clearly-labelled
``EML translation'' research-note appendix.  Readers using or
proving things about UNDERWOOD do not need to learn EML.
...
expansion of the ellipse perimeter near $b = 0$, and minimality of
the resulting basis follows from the standard
linear-independence-and-closure arguments on the augmented basis
(\S~\ref{sec:basis-opt}).  No gate function, no fitted exponential
